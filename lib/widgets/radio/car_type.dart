...
class CarTypeRadioWidgets extends StatelessWidget {
  final int count;
  final String title;
  final List<RadioModel> labelList;
  const CarTypeRadioWidgets({Key? key,
  required this.title,
  required this.count,
              ...
              itemBuilder: (context,index){
                return Row(
                  children: [
                    ...
                    Obx(
                      () {
                        return Radio<CarType>(
                          value: labelList[index].value, 
                          groupValue: _controller.carType.value, 
                          onChanged: (value) => _controller.changeCarType(value!),
                          );
                      }
                    ),
                  ],
